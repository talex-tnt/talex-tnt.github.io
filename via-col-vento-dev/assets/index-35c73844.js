...
`,Routes=()=>{const e="hid"in navigator||OVERRIDE_HID_CHECK,{hideCanvasScene:t,hideConfigureScene:o}=useCanvasConfig(),n=reactExports.useRef();reactExports.useEffect(()=>{const u=getPanelElement("top-panel"),_=getPanelElement("bottom-panel"),C=getResizeHandleElement("resize-handle");n.current={topPanelElement:u,bottomPanelElement:_,resizeHandleElement:C}},[]);const r=o?"50%":"100%",s=useAppSelector(getRenderMode),i=reactExports.useMemo(()=>PANES.map(u=>jsx(Route,{component:u.component,path:u.path},u.key)),[]),a=s==="2D",c=a?CanvasRouter:CanvasRouter$1,l=reactExports.useState({clearTestKeys:()=>{}});reactExports.useEffect(()=>{if(n.current){const{topPanelElement:u,bottomPanelElement:_,resizeHandleElement:C}=n.current;u&&_&&C&&(o?(u.style.display=a?"none":"block",_.style.display=a?"block":"none",C.style.display="none"):(u.style.display=t?"none":"block",_.style.display="block",C.style.display=t?"none":"block"))}},[t,o]);const d="/via-col-vento-dev";return jsx(Fragment,{children:jsxs(TestContext.Provider,{value:l,children:[jsx(GlobalStyle,{}),jsxs(Router,{base:d,children:[e&&jsx(UnconnectedGlobalMenu,{}),jsxs(PanelGroup,{direction:"vertical",children:[jsx(Panel,{id:"top-panel",children:jsx(c,{height:r})}),jsx(PanelResizeHandle,{id:"resize-handle"}),jsx(Panel,{id:"bottom-panel",children:jsx(Home,{hasHIDSupport:e,children:i})})]})]})]})})},Root=()=>jsx(Provider_default,{store,children:jsx(Routes,{})}),elem=document.getElementById("root");elem&&(createRoot(elem).render(jsx(Root,{})),document.documentElement.dataset.themeMode=getThemeModeFromStore(),updateCSSVariables(getThemeNameFromStore()));
