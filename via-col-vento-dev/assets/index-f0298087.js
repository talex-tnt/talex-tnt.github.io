...
`,KeyboardRouteGroup=styled.div`
  ...
  align-items: center;
  justify-content: center;
`,CanvasRouter=({height:e=500})=>{const[t]=useLocation(),o=reactExports.useRef(document.body),n=reactExports.useRef(null),r=useAppSelector(getLoadProgress),{progress:s}=useProgress(),a=useAppDispatch(),i=useSize(n),c=useSize(o),l=Object.values(useAppSelector(getCustomDefinitions)),d=useAppSelector(getSelectedDefinition),u=useAppSelector(getDesignDefinitionVersion),C=useAppSelector(getSelectedTheme),_=reactExports.useMemo(()=>C[dist.KeyColorType.Accent].c,[C]),K=t==="/"&&(!d||r!==1),p=reactExports.useMemo(()=>l.filter(x=>x[u]),[l,u]),h=t==="/design"&&!p.length,y=t==="/"&&(!d||(r+s/100)/2!==1),g=reactExports.useCallback(()=>{a(updateSelectedKey(null))},[a]),S=!("hid"in navigator||OVERRIDE_HID_CHECK)||["/settings","/errors"].includes(t)||h||y,b=useAppSelector(getConfigureKeyboardIsSelectable),T=K;return jsx(Fragment,{children:jsx("div",{style:{height:e,width:"100%",top:0,transform:S?T?c?`translateY(${-300+c.height/2}px)`:"":"translateY(-500px)":"",position:S&&!T?"absolute":"relative",overflow:"visible",zIndex:2,visibility:S&&!T?"hidden":"visible"},onClick:x=>{x.target.nodeName!=="CANVAS"&&a(clearSelectedKey())},ref:n,children:S?null:jsxs(Fragment,{children:[jsx(KeyboardBG,{onClick:g,$color:_,$visible:!T}),jsx(KeyboardGroup,{containerDimensions:i,configureKeyboardIsSelectable:b,loadProgress:r})]})})})},getRouteX=e=>{switch(e){case"/debug":return-300;case"/design":return-200;case"/test":return-100;case"/":return 0;default:return-300}},KeyboardGroupContainer=styled.div`
  z-index: 2;
  display: block;
  white-space: nowrap;
  height: 100%;
