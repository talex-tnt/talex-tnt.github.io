...
`,componentJoin=(e,t)=>e.reduce((o,n,r)=>(r&&o.push({...t,key:r.toString()}),o.push(n),o),[]),KeycodeMap=getKeycodes().flatMap(e=>e.keycodes).reduce((e,t)=>({...e,[t.code]:t}),{}),optimizeKeycodeSequence=e=>pipeline(e,convertCharacterTaps,trimLastWait,mergeConsecutiveWaits,foldKeydownKeyupKeys,convertToCharacterStreams),cleanKeycodeSequence=e=>pipeline(e,mergeConsecutiveWaits),MacroRecorder=({selectedMacro:e,setUnsavedMacro:t,saveMacro:o,undoMacro:n,isDelaySupported:r})=>{const[s,a]=reactExports.useState(!0),[i,c]=reactExports.useState(!1),[l,d]=reactExports.useState(!1),[u,C]=reactExports.useState(!!document.fullscreenElement),{smartOptimizeEnabled:_,recordDelaysEnabled:h}=useAppSelector(getMacroEditorSettings),p=useDispatch(),[K,y]=useKeycodeRecorder(i,h),g=reactExports.useRef(null),m=reactExports.useCallback(async A=>{c(A),A?(await navigator.keyboard.lock(),y([]),a(!1),d(!0)):(navigator.keyboard.unlock(),_&&y(optimizeKeycodeSequence(K)),d(!1))},[K,c]),S=reactExports.useCallback(()=>{o(""),a(!0),d(!1)},[y,o]),M=reactExports.useCallback(()=>{n(),y([]),a(!0),d(!1)},[n]);reactExports.useEffect(()=>{a(!0),d(!1),y([])},[e]);const b=()=>[...s?e??[]:K],f=reactExports.useMemo(()=>{let A,L=b();return s||!l||!_?A=L:A=optimizeKeycodeSequence(L),A},[K,s,_,l,e]);reactExports.useEffect(()=>{f&&t(sequenceToExpression(f))},[f]);const T=reactExports.useCallback(()=>{s&&a(!1)},[s]),D=reactExports.useCallback(A=>{const L=b();L.splice(A,1),y(cleanKeycodeSequence(L)),T()},[f,e,K,s]),v=reactExports.useCallback((A,L)=>{const I=b();I.splice(A,1,[RawKeycodeSequenceAction.Delay,L]),y(cleanKeycodeSequence(I)),T()},[f,e,K,s]),B=reactExports.useMemo(()=>componentJoin(f.map(([A,L],I)=>{const P=getSequenceItemComponent(A);return jsx(Deletable,{index:I,deleteItem:D,disabled:i,children:RawKeycodeSequenceAction.Delay!==A?jsx(P,{children:A===RawKeycodeSequenceAction.CharacterStream?componentJoin(String(L).split(" ").map((w,H)=>jsx("span",{children:w},H)),jsx("span",{style:{fontFamily:"fantasy, cursive, monospace"},children:"␣"})):Array.isArray(L)?L.map(w=>getSequenceLabel(KeycodeMap[w])??w).join(" + "):getSequenceLabel(KeycodeMap[L])}):jsx(WaitInput,{index:I,value:Number(L),updateValue:v})},`${I}-${A}`)}),jsx(SequenceLabelSeparator,{})),[f]);reactExports.useEffect(()=>{const A=()=>{C(!!document.fullscreenElement)};return document.documentElement.addEventListener("fullscreenchange",A),()=>{m(!1),document.documentElement.removeEventListener("fullscreenchange",A)}},[C]);const j=reactExports.useCallback(()=>{document.fullscreenElement?document.exitFullscreen&&(m(!1),document.exitFullscreen()):document.documentElement.requestFullscreen()},[m]);return jsxs(Fragment,{children:[jsx(MacroSequenceContainer,{ref:g,$isModified:!s,children:B.length?B:jsx(NoMacroRecorded,{children:"No macro recorded yet..."})}),jsx("div",{style:{border:"none",maxWidth:960,width:"100%",display:"flex",justifyContent:"center",transform:"translate(-0px, -21px)"},children:jsx(MacroEditControls,{isFullscreen:u,isEmpty:!e||!e.length,optimizeRecording:_,recordDelays:h,isRecording:i,addText:()=>{},deleteMacro:S,toggleOptimizeRecording:()=>{p(setMacroEditorSettings({smartOptimizeEnabled:!_}))},toggleRecordDelays:()=>{p(setMacroEditorSettings({recordDelaysEnabled:!h}))},toggleFullscreen:j,undoChanges:M,saveChanges:()=>o(),hasUnsavedChanges:!s,recordingToggleChange:m,isDelaySupported:r})})]})},TextArea=styled.textarea`
  box-sizing: border-box;
  background: var(--bg_control);
  padding: 5px 10px;
